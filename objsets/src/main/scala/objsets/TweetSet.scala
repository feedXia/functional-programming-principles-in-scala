...
abstract class TweetSet extends TweetSetInterface:
  ...
  def mostRetweetedHelper(mostRetweeted: Tweet): Tweet = ???
...
class Empty extends TweetSet:
  ...
  override def mostRetweetedHelper(mostRetweeted: Tweet): Tweet = mostRetweeted
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  override def mostRetweetedHelper(mostRetweeted: Tweet): Tweet =
    if elem.retweets > mostRetweeted.retweets then
      right.mostRetweetedHelper(left.mostRetweetedHelper(elem))
    else right.mostRetweetedHelper(left.mostRetweetedHelper(mostRetweeted))
...
object GoogleVsApple:
  ...
  def filterAllTweetsByWords(words: List[String]) =
    TweetReader.allTweets.filter(t => words.exists(t.text.contains(_)))

  lazy val googleTweets: TweetSet = filterAllTweetsByWords(google)
  lazy val appleTweets: TweetSet = filterAllTweetsByWords(apple)
